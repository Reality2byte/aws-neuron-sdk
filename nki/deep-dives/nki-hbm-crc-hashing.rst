.. meta::
   :description: Clarification of HBM CRC-based hashing and why large contiguous DMA payloads are still required for peak bandwidth in NKI.
   :keywords: NKI, HBM, CRC hashing, DMA, bandwidth, payload size, SDMA, AWS Neuron, Trainium
   :date-modified: 03/31/2026

.. _hbm-crc-hashing-clarification:

=====================================================
HBM CRC-Based Hashing and DMA Bandwidth Requirements
=====================================================
...
on the per-transfer payload size seen by the SDMA (System DMA) engines.
...
- Eliminate SDMA packets-per-second (PPS) bottlenecks caused by small
  transfers.
...
the engines spend more time on overhead than on data movement, and the SDMA
...
- **≥ 4 KiB per partition (target for full saturation):** Fully amortizes
  per-packet overhead and saturates the SDMA engines.
